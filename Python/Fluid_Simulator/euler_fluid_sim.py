...
DIFFUSION = 0.0000    # Taxa de difusão (viscosidade). Um valor pequeno torna o fluido "espesso".
VISCOSITY = 0.0000    # Viscosidade do campo de velocidade.
...
im = ax.imshow(fluid.density, cmap='viridis', origin='lower', interpolation='bilinear')
...
def update(frame):
    fluid.step()
    im.set_data(fluid.density.T) # Transpor para corresponder à orientação do imshow
    im.set_clim(0, np.max(fluid.density))
    return [im]
